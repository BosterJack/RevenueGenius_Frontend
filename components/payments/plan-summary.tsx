   ...
   {plan &&  <Card className="shadow-md sticky top-8">
      ...
      <CardContent className="pt-6">
        ...
        {paymentStatus === "error" && !error && (
          <Alert className="mt-4 bg-red-50 text-red-800 border-red-200">
            <AlertCircle className="h-4 w-4" />
            <AlertDescription>
              Échec du paiement. Veuillez réessayer ou utiliser une autre méthode de paiement.
            </AlertDescription>
          </Alert>
        )}
      </CardContent>
      <CardFooter className="flex flex-col space-y-4 pt-4 border-t">
        ...
        <Button
          ...
          size="lg"
          onClick={handleCreatePaymentIntent}
          disabled={isLoading || isCreatingPaymentIntent || !selectedPaymentMethodId || !stripe}
        >
          {isLoading || isCreatingPaymentIntent ? (
            <span className="flex items-center">
              <Loader2 className="mr-2 h-4 w-4 animate-spin" />

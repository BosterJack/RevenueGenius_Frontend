...
const {addLeadInteraction,isAddingLeadInteraction,deleteLead,leadsInterractions,leads:leadInteractionsData,updateLeadInterraction,deleteLeadInterraction,isDeletingLeadInterraction,isUpdatingLeadInterraction}=useLeads()
  const [isAddDialogOpen, setIsAddDialogOpen] = useState(false)
  const [isEditDialogOpen, setIsEditDialogOpen] = useState(false)
  const [selectedInteraction, setSelectedInteraction] = useState<any>(null)
  const [selectedLead, setSelectedLead] = useState<string>(Array.isArray(leadInteractionsData) && leadInteractionsData[0]?.id || "") // Default lead ID
...
// })
  ...
  function onEditSubmit(data: InteractionFormValues) {
    updateLeadInterraction({id:selectedLead,interactionId:selectedInteraction?.id,data})
...
const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false)
 function handleDeleteForecast(interaction: any) {
  ...
  function confirmDelete() {
    deleteLeadInterraction({id:selectedLead,interactionId:selectedInteraction?.id})
    ...
    <div className="space-y-6">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-col">
                      ...
                      <FormControl>
                        <Input type="date" placeholder="Interaction details..." {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Textarea placeholder="Interaction details..." {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                />
                <Button disabled={isAddingLeadInteraction} type="submit" className="w-full">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Select a lead to view their interactions</CardDescription>
        </CardHeader>
        <CardContent>
          <Select value={selectedLead} onValueChange={setSelectedLead}>
            ...
            <SelectContent>
              {Array.isArray(leadInteractionsData) && leadInteractionsData.map((lead) => (
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>All recorded interactions with the selected lead</CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
            ...
            <TableBody>
              {Array.isArray(data) && data?.map((interaction) => (
          ...
          { selectedInteraction && selectedInteraction && (
            <Form {...editForm}>
              <form onSubmit={editForm.handleSubmit(onEditSubmit)} className="space-y-4">
                ...
                />
                <Button disabled={isUpdatingLeadInterraction} type="submit" className="w-full">
                  {isUpdatingLeadInterraction && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                  Update Interaction
                </Button>
              </form>
            </Form>
      ...
      <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <Button onClick={confirmDelete} disabled={isDeletingLeadInterraction} className="bg-red-600 hover:bg-red-700">
              Delete
            </Button>
          </AlertDialogFooter>
        </AlertDialogContent>

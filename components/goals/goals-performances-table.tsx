...
export function GoalsTable() {
  ...
  const sortedGoals = Array.isArray(goals)
    ? [...goals].sort((a, b) => {
        if (["progress", "progress_percentage", "current_value", "target_value"].includes(sortColumn)) {
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead className="cursor-pointer" onClick={() => handleSort("type_display")}>
              Type
            </TableHead>
            {/* <TableHead className="cursor-pointer" onClick={() => handleSort("is_completed")}>
              Status
            </TableHead> */}
            <TableHead className="cursor-pointer text-right" onClick={() => handleSort("progress_percentage")}>
              Progress
            </TableHead>
            <TableHead className="cursor-pointer text-right" onClick={() => handleSort("current_value")}>
              Current Value
            </TableHead>
            <TableHead className="cursor-pointer text-right" onClick={() => handleSort("target_value")}>
              Target Value
            </TableHead>
            <TableHead className="cursor-pointer" onClick={() => handleSort("target_date")}>
              Deadline
            </TableHead>
        ...
        <TableBody>
          {paginatedGoals.map((goal) => (
            <TableRow
              key={goal.id}
              //@ts-ignore
              className={`cursor-pointer ${selected?.id === goal.id ? "bg-red-50" : ""}`}
            ...
            >
                  ...
                  <div className="w-16 bg-gray-200 rounded-full h-2">
                    <div
                      className={`h-2 rounded-full ${
                        goal.is_completed ? "bg-green-500" : goal.status === "at-risk" ? "bg-amber-500" : "bg-blue-500"
                      }`}
              ...
              <TableCell className="text-right">{goal.current_value}</TableCell>
